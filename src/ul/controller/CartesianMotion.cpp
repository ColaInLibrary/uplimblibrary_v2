...
bool CartesianMotion::speedL(::std::vector<::ul::math::Vector6> init_xd, ::std::vector<::ul::math::Vector6> init_xdd, const ::std::vector<::ul::math::Vector6> &trg_xd, const double& acceleration, const double& time) {
  assert(time > 0.0);
  this->max_length = 0;
  this->robot.setPosition(this->driver.getLastCommandJointPosition());
  ::std::vector<::ul::math::Matrix> J(2), J_temp(2);
  ...
  for (int i = 0; i < arm_num; ++i) {
    for (int j = 0; j < cartesian_dim; ++j) {
      ...
      this->speedL_ratio_[i][j] = trajectory_time[i][j]/max_time;
    }
  }
  ...
  for (int i = 0; i < arm_num; ++i) {
    for (int j = 0; j < cartesian_dim; ++j) {
      ...
      // std::cout << "\n Joint[" << i << "][" << j << "]: " << std::endl;
      this->speedL_sv_[i][j].replan(this->speedL_ratio_[i][j]);
      trajectory_length[i][j] = ::std::ceil(this->speedL_sv_[i][j].t()/this->driver.getUpdateRate());
    }
  }
