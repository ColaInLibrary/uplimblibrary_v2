#ifndef UL_MATH_POLYNOMIAL_H
#define UL_MATH_POLYNOMIAL_H
...
class Polynomial : public Function<T> {
 public:
  Polynomial() : Function<T>(0, 0), c() {}

  Polynomial(const ::std::size_t& degree) : Function<T>(0, 0), c(degree + 1) {}

  virtual ~Polynomial() {}

  template <typename U = T>
  static Polynomial CubicAtRest(const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& y0,
  ...
  template <typename U = T>
  static Polynomial QuinticAtRest(const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& y0,
                                  const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& y1,
                                  const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& ydmax,
                                  const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& yddmax,
                                  const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& ydddmax) {
  ...
  template <typename U = T>
  static Polynomial SepticAtRest(const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& y0,
                                 const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& y1,
                                 const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& ydmax,
                                 const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& yddmax,
                                 const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& ydddmax) {
  ...
  T& coefficient(const ::std::size_t& i) { return this->c[i]; }

  const T& coefficient(const ::std::size_t& i) const { return this->c[i]; }

  ::std::size_t degree() const { return this->c.size() - 1; }
  ...
  friend Polynomial operator+(const T& lhs, const Polynomial& rhs) { return Polynomial(rhs) += lhs; }

  friend Polynomial operator+(const Polynomial& lhs, const T& rhs) { return Polynomial(lhs) += rhs; }
  ...
  friend Polynomial operator*(const Real& lhs, const Polynomial& rhs) { return Polynomial(rhs) *= lhs; }

  friend Polynomial operator*(const Polynomial& lhs, const Real& rhs) { return Polynomial(lhs) *= rhs; }
  ...
  Polynomial& operator/=(const Real& rhs) { return *this *= 1 / rhs; }
  ...
  static ::std::vector<Real> realRoots(const ::std::vector<Real>& c) {
    ...
    if (degree < 1) {
      ...
      if (::std::abs(p) <= ::std::numeric_limits<Real>::epsilon() && ::std::abs(q) <= ::std::numeric_limits<Real>::epsilon()) {
        ...
        roots[2] = -sqrtP - back;
        return roots;
      } else if (::std::abs(4 * ::std::pow(p, 3) + 27 * ::std::pow(q, 2)) <= ::std::numeric_limits<Real>::epsilon() &&
                 ::std::abs(p) > ::std::numeric_limits<Real>::epsilon()) {
        ::std::vector<Real> roots(2);
