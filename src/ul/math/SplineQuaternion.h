#ifndef UL_MATH_QUATERNIONSPLINE_H
#define UL_MATH_QUATERNIONSPLINE_H
...
class Spline<Quaternion> : public Function<Quaternion> {
 public:
  ...
  Spline<Quaternion>() : Function<Quaternion>(0, 0), polynomials() {}
  ...
  static Spline<Quaternion> CubicFirst(const ::std::vector<Real>& x, const ::std::vector<Quaternion>& y, const Vector3& yd0, const Vector3& yd1) {
    ...
    do {
      ...
      for (::std::size_t i = 1; i < n - 1; ++i) {
        ...
        b[i] = 4 / dx[i - 1] + 4 / dx[i];
        c[i] = 2 / dx[i];
        d[i] = (6 * e[i - 1] * dtheta[i - 1]) / ::std::pow(dx[i - 1], 2) + (6 * e[i] * dtheta[i]) / ::std::pow(dx[i], 2) -
               R(e[i - 1], dtheta[i - 1], omegaprev[i]);
      }
  ...
  static Spline<Quaternion> TrapezoidalAccelerationAtRest(const Quaternion& y0, const Quaternion& y1, const Vector3& vmax, const Vector3& amax,
                                                          const Vector3& jmax) {
    using ::std::abs;
    using ::std::atan2;
    ...
    for (::std::size_t i = 0; i < f2.size(); ++i) {
      ...
      ::ul::math::Real dx = f2[i].upper() - f2[i].lower();
      ::ul::math::Real dx2 = dx * dx;
      ::ul::math::Real dx3 = dx2 * dx;
  ...
  Polynomial<Quaternion>& at(const ::std::size_t& i) { return this->polynomials.at(i); }

  const Polynomial<Quaternion>& at(const ::std::size_t& i) const { return this->polynomials.at(i); }

  Polynomial<Quaternion>& back() { return this->polynomials.back(); }

  const Polynomial<Quaternion>& back() const { return this->polynomials.back(); }

  Iterator begin() { return this->polynomials.begin(); }

  ConstIterator begin() const { return this->polynomials.begin(); }

  void clear() { this->polynomials.clear(); }

  Spline<Quaternion>* clone() const { return new Spline<Quaternion>(*this); }
  ...
  Polynomial<Quaternion>& front() { return this->polynomials.front(); }

  const Polynomial<Quaternion>& front() const { return this->polynomials.front(); }
  ...
  Polynomial<Quaternion>& operator[](const ::std::size_t& i) { return this->polynomials[i]; }

  const Polynomial<Quaternion>& operator[](const ::std::size_t& i) const { return this->polynomials[i]; }
...
#endif  // UL_MATH_QUATERNIONSPLINE_H
