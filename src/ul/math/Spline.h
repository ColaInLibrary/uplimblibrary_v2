#ifndef UL_MATH_SPLINE_H
#define UL_MATH_SPLINE_H
...
class Spline : public Function<T> {
 public:
  ...
  Spline() : Function<T>(0, 0), polynomials() {}
  ...
  static Spline LinearQuartic(const Container1& x, const Container2& y, const Real& quarticInterval) {
...

    {
      ...
      Polynomial<T> quartic =
          Polynomial<T>::QuarticFirstSecond(yBeforeQuartic, y[n - 1], yd, TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim), quarticInterval);
      f.push_back(quartic);
    }
  ...
  static Spline LinearSextic(const Container1& x, const Container2& y, const Real& sexticInterval) {
...

    {
      ...
      Polynomial<T> sextic = Polynomial<T>::SexticFirstSecondThird(y[0], yBeforeLinear, TypeTraits<T>::Zero(dim), yd, TypeTraits<T>::Zero(dim),
                                                                   TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim), sexticInterval);
      f.push_back(sextic);
      Polynomial<T> linear = Polynomial<T>::Linear(yBeforeLinear, yAfterLinear, linearInterval);
      f.push_back(linear);
    }
    ...
    for (::std::size_t i = 1; i < n - 1; ++i) {
      ...
      Polynomial<T> sextic = Polynomial<T>::SexticFirstSecondThird(yBeforeSextic, yBeforeLinear, ydPrev, ydNext, TypeTraits<T>::Zero(dim),
                                                                   TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim), sexticInterval);
      f.push_back(sextic);
      Polynomial<T> linear = Polynomial<T>::Linear(yBeforeLinear, yAfterLinear, linearInterval);
      f.push_back(linear);
...

    {
      ...
      Polynomial<T> sextic = Polynomial<T>::SexticFirstSecondThird(yBeforeSextic, y[n - 1], yd, TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim),
                                                                   TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim), sexticInterval);
      f.push_back(sextic);
    }
  ...
  static Spline LinearSexticPercentage(const Container1& x, const Container2& y, const Real& sexticPercent) {
    ...
    for (::std::size_t i = 1; i < n - 2; ++i) {
      ...
      Polynomial<T> sextic = Polynomial<T>::SexticFirstSecondThird(yBeforeSextic, yBeforeLinear, ydPrev, ydNext, TypeTraits<T>::Zero(dim),
                                                                   TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim), sexticIntervalPrev);
      f.push_back(sextic);
      Polynomial<T> linear = Polynomial<T>::Linear(yBeforeLinear, yAfterLinear, linearInterval);
      f.push_back(linear);
    }
  ...
  static Spline QuarticLinearQuarticAtRest(const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& q0,
   ...
   */
  template <typename U = T>
  static Spline QuarticLinearQuarticAtRest(const typename ::std::enable_if<::std::is_class<U>::value, U>::type& q0,
    ...
    assert(q0.size() >= 1 && q0.size() == q1.size() && q0.size() == vmax.size() && q0.size() == amax.size() &&
    ...
    Polynomial<T> acc =
        Polynomial<T>::QuarticFirstSecond(q0, q0 + sign * xAfterAcc, TypeTraits<T>::Zero(dim), sign * vReached, TypeTraits<T>::Zero(dim), tAccMax);
    f.push_back(acc);
    ...
    Polynomial<T> dec =
        Polynomial<T>::QuarticFirstSecond(q1 - sign * xAfterAcc, q1, sign * vReached, TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim), tAccMax);
    f.push_back(dec);
   ...
   */
  template <typename U = T>
  static Spline SexticLinearSexticAtRest(const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& q0,
                                         const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& q1,
                                         const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& vmax,
                                         const typename ::std::enable_if<::std::is_floating_point<U>::value, U>::type& amax) {
   ...
   */
  template <typename U = T>
  static Spline SexticLinearSexticAtRest(const typename ::std::enable_if<::std::is_class<U>::value, U>::type& q0,
                                         const typename ::std::enable_if<::std::is_class<U>::value, U>::type& q1,
                                         const typename ::std::enable_if<::std::is_class<U>::value, U>::type& vmax,
                                         const typename ::std::enable_if<::std::is_class<U>::value, U>::type& amax) {
    assert(q0.size() >= 1 && q0.size() == q1.size() && q0.size() == vmax.size() && q0.size() == amax.size() &&
    ...
    Polynomial<T> acc = Polynomial<T>::SexticFirstSecondThird(q0, q0 + sign * xAfterAcc, TypeTraits<T>::Zero(dim), sign * vReached, TypeTraits<T>::Zero(dim),
                                                              TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim), tAccMax);
    f.push_back(acc);
    ...
    Polynomial<T> dec = Polynomial<T>::SexticFirstSecondThird(q1 - sign * xAfterAcc, q1, sign * vReached, TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim),
                                                              TypeTraits<T>::Zero(dim), TypeTraits<T>::Zero(dim), tAccMax);
    f.push_back(dec);
  ...
  static Spline TrapezoidalAccelerationAtRest(const T& q0, const T& q1, const T& vmax, const T& amax, const T& jmax) {
    //				assert((vmax > 0).all() && (amax > 0).all() && (jmax > 0).all() && "TrapezoidalAccelerationAtRest: velocity, acceleration and
    //jerk limits must be positive.");
  ...
  Polynomial<T>& at(const ::std::size_t& i) { return this->polynomials.at(i); }

  const Polynomial<T>& at(const ::std::size_t& i) const { return this->polynomials.at(i); }

  Polynomial<T>& back() { return this->polynomials.back(); }

  const Polynomial<T>& back() const { return this->polynomials.back(); }

  Iterator begin() { return this->polynomials.begin(); }

  ConstIterator begin() const { return this->polynomials.begin(); }
  ...
  Spline* clone() const { return new Spline(*this); }
  ...
  bool empty() { return this->polynomials.empty(); }

  Iterator end() { return this->polynomials.end(); }

  ConstIterator end() const { return this->polynomials.end(); }

  Polynomial<T>& front() { return this->polynomials.front(); }

  const Polynomial<T>& front() const { return this->polynomials.front(); }
  ...
  Polynomial<T>& operator[](const ::std::size_t& i) { return this->polynomials[i]; }

  const Polynomial<T>& operator[](const ::std::size_t& i) const { return this->polynomials[i]; }
  ...
  ::std::size_t size() const { return this->polynomials.size(); }

  ReverseIterator rbegin() { return this->polynomials.rbegin(); }

  ConstReverseIterator rbegin() const { return this->polynomials.rbegin(); }

  ReverseIterator rend() { return this->polynomials.rend(); }

  ConstReverseIterator rend() const { return this->polynomials.rend(); }
